...
BATCH_LOG=$(mktemp)
# run with a timeout and capture logs to make failures actionable in CI
timeout_seconds=${VOACAPL_BATCH_TIMEOUT:-120}
if ! timeout ${timeout_seconds}s "$VOACAPL_BIN" -s "$ITSHFBC" batch > "$BATCH_LOG" 2>&1; then
    echo "ERROR: voacapl batch failed or timed out after ${timeout_seconds}s"
    sed -n '1,400p' "$BATCH_LOG"
    rm -f "$BATCH_LOG"
    fail "batch invocation failed"
fi
# expected completion text may vary across voacapl versions — check common phrasing
if ! grep -q -i "complete\|completed\|end of batch" "$BATCH_LOG"; then
    echo "ERROR: batch run did not report expected completion text"
    sed -n '1,400p' "$BATCH_LOG"
    rm -f "$BATCH_LOG"
    fail "batch run did not report completion"
fi
rm -f "$BATCH_LOG"
